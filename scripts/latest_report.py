...
PATTERN = re.compile(r"^(CK-\d{4})_R(\d{3})\.md$")


def resolve_latest(base_dir: Path, work_item: str) -> Path:
    target_dir = base_dir / "qa" / "reports"
    if not target_dir.is_dir():
        raise FileNotFoundError(f"report directory not found: {target_dir}")
    ...
    if best_path is None:
        raise FileNotFoundError(f"no report found for work item {work_item}")
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: latest_report.py CK-XXXX", file=sys.stderr)
        return 2
